...
def global_rank(all_cards):
    ...
    prompt = (
        ...
        "SECONDARY signal: recency — but use it carefully. RSS timestamps update when articles are edited, so a recent timestamp does not always mean a story just broke.\n"
        "Apply this weighting:\n"
        "1. Stories with direct US impact (economy, security, foreign policy, domestic policy): highest priority\n"
        "2. Major geopolitical developments that affect global stability, oil, trade, or US allies: very high\n"
        "3. Government resignations, cabinet changes, national security developments: high\n"
        "4. Regional political events with limited US relevance (e.g. opposition crackdowns in non-allied countries): lower — these belong in World but should rarely lead the front page\n"
        "5. Follow-up stories (new details on a previous day's event): rank below genuinely new stories\n"
